...
for (let Film of Films) {
  ...
  toHoursAndMinutes =
    Math.floor(Film.totalTimeInminutes / 60) +
    "h " +
    (Film.totalTimeInminutes % 60) +
    "min";
  watchedPersent = Math.round(
    (Film.watchedTimeInMinutes / Film.totalTimeInminutes) * 100
  );

  htmlElements0 += `<div class="col col-sm-12 col-md-6 col-lg-4">
...
<br> le temps total visionné de la vidéothèque est:<br> ${totalTimewatched} minutes équivalent:  ${
  Math.floor(totalTimewatched / 60) + "h " + (totalTimewatched % 60)
} minutes`;
